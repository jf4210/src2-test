...
int main(int argc, const char *argv[])
{
    ...
    else if (opt_do_extract == 1)
    {
        ...
        if (filename_to_extract == NULL)
            ret = do_extract_all(uf, opt_do_extract_withoutpath, opt_overwrite, password, TODO);
        else
			ret = do_extract_onefile(uf, filename_to_extract, opt_do_extract_withoutpath, opt_overwrite, password, pBaseDir);
    }
