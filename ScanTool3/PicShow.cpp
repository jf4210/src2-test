...
BOOL CPicShow::OnInitDialog()
{
	...
	if (!m_pPicDlg)
	{
		m_pPicDlg = new CPicDlg();
		m_pPicDlg->Create(CPicDlg::IDD, this);
...
void CPicShow::InitCtrlPosition()
{
	...
	const int nGap = 1;
#ifdef TEST_PICSHOW
	if (m_pPicDlg->GetSafeHwnd())
	{
		if (!m_bShowScrolH && !m_bShowScrolV)
			m_pPicDlg->MoveWindow(0, 0, rt.Width(), rt.Height());
		else if (!m_bShowScrolH)
			m_pPicDlg->MoveWindow(0, 0, rt.Width() - nScrollV_W - nGap, rt.Height());
		else if (!m_bShowScrolV)
			m_pPicDlg->MoveWindow(0, 0, rt.Width(), rt.Height() - nScrollH_H - nGap);
		else
			m_pPicDlg->MoveWindow(0, 0, rt.Width() - nScrollV_W - nGap, rt.Height() - nScrollH_H - nGap);
	}
#else
...
#ifdef TEST_PICSHOW
	m_pPicDlg->m_picShow.ShowImage_Rect_roi(m_src_img, pt);
#else
...
void CPicShow::ShowPic(cv::Mat& imgMat, cv::Point pt /*= cv::Point(0,0)*/, float fShowPer /*= 1.0*/, int nDirection /*= -1*/)
{
	...
	CRect rcPic;
	m_pPicDlg->m_picShow.GetClientRect(&m_client);
	...
	m_ptBase = pt;
	m_src_img = imgMat;
	//TRACE("CPicShow::ShowPic--> 显示起点(%d,%d)\n", pt.x, pt.y);
	m_pPicDlg->ShowPic(imgMat, pt, m_fScale, nDirection);
#else
	CRect rcDlg;
	this->GetClientRect(&rcDlg);
...
#ifdef TEST_PICSHOW
	m_pPicDlg->m_picShow.ShowImage_roi(*pShowMat, 0);
#else
...
{
#ifdef TEST_PICSHOW
	m_pPicDlg->SetShowTracker(bShowH, bShowV, bShowSN, bShowZgt);
#else
