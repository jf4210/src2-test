...
extern "C" BOOL UnskinWndScroll(
	...
	if (frame) {
		...
		SetWindowLongPtr(pWnd->m_hWnd,GWLP_USERDATA,frame->m_ptr);
...
BOOL CSkinScrollWnd::SkinWindow(
	...
	this->CreateEx(dw_frm_style_ex,AfxRegisterWndClass(NULL),_T("SkinScrollBarFrame"),dw_frm_style,rc_frm,parent,u_id);

	m_wnd_limit.Create(NULL, _T("LIMIT"),WS_CHILD|WS_VISIBLE,CRect(0,0,0,0),this,200);
	...
	m_ptr=SetWindowLongPtr(pWnd->m_hWnd,GWLP_USERDATA,(LONG_PTR)this);
